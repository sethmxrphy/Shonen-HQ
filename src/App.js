...
function NinjaAvatar({ level, size = 160 }) {
  ...
  const hasCape = level >= 6, hasMask = level >= 10;
  const hasHelmet = level >= 15, hasAura = level >= 3;
  const hasDual = level >= 10, hasScar = level >= 6;

  // Skin tone — masked at level 10+
  const skinColor = hasMask ? tier.cloakColor : "#C8956C";
  const hairColor = level >= 10 ? "#0A0A0A" : level >= 6 ? "#1A0A0A" : "#2A1A0A";

  return (
    <div style={{ position:"relative", width:s, height:s, margin:"0 auto" }}>
      {/* Aura rings */}
      {level >= 15 && <div style={{ position:"absolute", inset:-10, borderRadius:"50%", border:`1.5px solid ${tier.rankColor}`, animation:"legendAura 2s ease-in-out infinite", opacity:0.45 }}/>}
      {level >= 20 && <div style={{ position:"absolute", inset:-22, borderRadius:"50%", border:`1px solid ${tier.rankColor}`, animation:"legendAura 3s ease-in-out infinite reverse", opacity:0.25 }}/>}
      {hasAura && <div style={{ position:"absolute", inset:0, borderRadius:"50%", background:`radial-gradient(ellipse at 50% 60%, ${tier.aura} 0%, transparent 68%)`, animation:"auraFloat 2.8s ease-in-out infinite" }}/>}
      {tier.particles && [0,1,2,3,4].map(i => (
        <div key={i} style={{ position:"absolute", width:3, height:3, borderRadius:"50%", background:tier.rankColor, top:`${18+i*13}%`, left:i%2===0?`${4+i*2}%`:`${76+i*2}%`, animation:`sparkle ${1.6+i*0.35}s ease-in-out infinite`, animationDelay:`${i*0.28}s`, boxShadow:`0 0 5px ${tier.rankColor}` }}/>
      ))}

      <svg width={s} height={s} viewBox={`0 0 ${s} ${s}`} style={{ position:"relative", zIndex:2 }}>
        <defs>
          <filter id="softShadow" x="-20%" y="-20%" width="140%" height="140%">
            <feDropShadow dx="0" dy="2" stdDeviation="2" floodColor="#00000066"/>
          </filter>
        </defs>
        ...
        {hasCape && (
          <path
            d={`M ${cx-14} ${s*0.41} C ${cx-28} ${s*0.55} ${cx-32} ${s*0.78} ${cx-18} ${s*0.95} Q ${cx} ${s*0.98} ${cx+18} ${s*0.95} C ${cx+32} ${s*0.78} ${cx+28} ${s*0.55} ${cx+14} ${s*0.41}`}
            fill={tier.cloakColor} stroke={`${tier.rankColor}55`} strokeWidth="0.8"
          />
        )}

        {/* ── LEGS ── */}
        {/* Left leg — slightly angled for dynamic stance */}
        <path d={`M ${cx-12} ${s*0.72} L ${cx-14} ${s*0.94} Q ${cx-16} ${s*0.97} ${cx-8} ${s*0.97} L ${cx-6} ${s*0.72}`}
          fill={level>=6?"#151020":"#1E2030"} filter="url(#softShadow)"
        />
        {/* Right leg */}
        <path d={`M ${cx+6} ${s*0.72} L ${cx+8} ${s*0.94} Q ${cx+16} ${s*0.97} ${cx+14} ${s*0.97} L ${cx+12} ${s*0.72}`}
          fill={level>=6?"#151020":"#1E2030"} filter="url(#softShadow)"
        />
        {/* Boot details */}
        {level >= 3 && <>
          <line x1={cx-15} y1={s*0.88} x2={cx-5} y2={s*0.88} stroke={tier.rankColor} strokeWidth="1" opacity="0.4"/>
          <line x1={cx+5} y1={s*0.88} x2={cx+15} y2={s*0.88} stroke={tier.rankColor} strokeWidth="1" opacity="0.4"/>
        </>}

        {/* ── TORSO ── */}
        <path d={`M ${cx-16} ${s*0.42} C ${cx-17} ${s*0.38} ${cx-14} ${s*0.35} ${cx} ${s*0.35} C ${cx+14} ${s*0.35} ${cx+17} ${s*0.38} ${cx+16} ${s*0.42} L ${cx+14} ${s*0.72} L ${cx-14} ${s*0.72} Z`}
          fill={tier.cloakColor} filter="url(#softShadow)"
        />
        {/* Vest panel */}
        {level >= 3 && (
          <path d={`M ${cx-10} ${s*0.43} C ${cx-10} ${s*0.40} ${cx} ${s*0.38} ${cx+10} ${s*0.43} L ${cx+8} ${s*0.65} L ${cx-8} ${s*0.65} Z`}
            fill={level>=10?"#180808":level>=6?"#0D1A18":"#0D180D"}
            stroke={tier.rankColor} strokeWidth="0.4" opacity="0.9"
          />
        )}
        {/* Chest symbol */}
        {level >= 6 && (
          <text x={cx} y={s*0.58} textAnchor="middle" fontSize={s*0.08}
            fill={tier.rankColor} opacity="0.85" fontFamily="serif" fontWeight="bold"
          >{level>=20?"卍":level>=15?"忍":level>=10?"闇":"忍"}</text>
        )}
        {/* Belt */}
        <rect x={cx-14} y={s*0.695} width={28} height={s*0.028} rx={2}
          fill={level>=6?tier.rankColor:"#3A2A14"} opacity="0.85"
        />
        {/* Belt buckle */}
        <rect x={cx-4} y={s*0.69} width={8} height={s*0.038} rx={1.5}
          fill={level>=6?tier.rankColor:"#4A3020"} stroke="#00000033" strokeWidth="0.5"
        />

        {/* ── LEFT ARM (relaxed, slightly out) ── */}
        <path d={`M ${cx-14} ${s*0.43} C ${cx-20} ${s*0.44} ${cx-26} ${s*0.52} ${cx-24} ${s*0.67} L ${cx-18} ${s*0.67} C ${cx-17} ${s*0.53} ${cx-12} ${s*0.47} ${cx-10} ${s*0.43} Z`}
          fill={tier.cloakColor}
        />
        {/* Left hand */}
        <ellipse cx={cx-21} cy={s*0.685} rx={4} ry={3} fill={skinColor}/>

        {/* ── RIGHT ARM (raised slightly, weapon arm) ── */}
        <path d={`M ${cx+14} ${s*0.43} C ${cx+20} ${s*0.42} ${cx+27} ${s*0.48} ${cx+26} ${s*0.62} L ${cx+20} ${s*0.64} C ${cx+18} ${s*0.51} ${cx+12} ${s*0.46} ${cx+10} ${s*0.43} Z`}
          fill={tier.cloakColor}
        />
        {/* Right hand */}
        <ellipse cx={cx+23} cy={s*0.645} rx={4} ry={3} fill={skinColor}/>

        {/* Arm wraps */}
        {level >= 3 && <>
          <line x1={cx-24} y1={s*0.58} x2={cx-16} y2={s*0.56} stroke={tier.rankColor} strokeWidth="1.2" opacity="0.4" strokeLinecap="round"/>
          <line x1={cx+16} y1={s*0.54} x2={cx+24} y2={s*0.55} stroke={tier.rankColor} strokeWidth="1.2" opacity="0.4" strokeLinecap="round"/>
        </>}

        {/* ── NECK ── */}
        <rect x={cx-5} y={s*0.33} width={10} height={s*0.045} rx={2} fill={skinColor}/>

        {/* ── HEAD ── */}
        {/* Hair / back of head */}
        {!hasHelmet && (
          <ellipse cx={cx} cy={s*0.24} rx={s*0.135} ry={s*0.14} fill={hairColor}/>
        )}
        {/* Face */}
        <ellipse cx={cx} cy={s*0.255} rx={s*0.115} ry={s*0.125} fill={skinColor} filter="url(#softShadow)"/>

        {/* Hair details */}
        {!hasHelmet && level < 10 && (
          <>
            <path d={`M ${cx-s*0.13} ${s*0.2} Q ${cx-s*0.16} ${s*0.11} ${cx-s*0.08} ${s*0.1} Q ${cx} ${s*0.09} ${cx+s*0.08} ${s*0.1} Q ${cx+s*0.14} ${s*0.12} ${cx+s*0.13} ${s*0.2}`}
              fill={hairColor}
            />
            {level >= 6 && (
              <path d={`M ${cx-s*0.13} ${s*0.18} Q ${cx-s*0.19} ${s*0.1} ${cx-s*0.14} ${s*0.05}`}
        ...
        {hasMask ? (
          /* Sleek half-mask */
          <>
            <path d={`M ${cx-s*0.11} ${s*0.245} Q ${cx-s*0.115} ${s*0.31} ${cx} ${s*0.315} Q ${cx+s*0.115} ${s*0.31} ${cx+s*0.11} ${s*0.245}`}
              fill={level>=15?"#0D0500":"#080812"} stroke={`${tier.rankColor}66`} strokeWidth="0.5"
            />
            {/* Glowing eyes */}
            <ellipse cx={cx-s*0.045} cy={s*0.245} rx={s*0.025} ry={s*0.016}
              fill={tier.rankColor} style={{filter:`drop-shadow(0 0 4px ${tier.rankColor})`}}
            />
            <ellipse cx={cx+s*0.045} cy={s*0.245} rx={s*0.025} ry={s*0.016}
              fill={tier.rankColor} style={{filter:`drop-shadow(0 0 4px ${tier.rankColor})`}}
            />
            {/* Mask lines */}
            <line x1={cx-s*0.05} y1={s*0.27} x2={cx+s*0.05} y2={s*0.27} stroke={tier.rankColor} strokeWidth="0.5" opacity="0.3"/>
          </>
        ) : (
          /* Normal eyes */
          <>
            <ellipse cx={cx-s*0.04} cy={s*0.25} rx={s*0.022} ry={s*0.018} fill="#0A0A0A"/>
            <ellipse cx={cx+s*0.04} cy={s*0.25} rx={s*0.022} ry={s*0.018} fill="#0A0A0A"/>
            <circle cx={cx-s*0.033} cy={s*0.248} r={s*0.006} fill="#fff" opacity="0.8"/>
            <circle cx={cx+s*0.047} cy={s*0.248} r={s*0.006} fill="#fff" opacity="0.8"/>
            {/* Nose */}
            <path d={`M ${cx-3} ${s*0.268} Q ${cx} ${s*0.276} ${cx+3} ${s*0.268}`}
              stroke="#9B7A5A" strokeWidth="0.8" fill="none"
            />
            {/* Mouth — slight smirk */}
            <path d={`M ${cx-4} ${s*0.284} Q ${cx+1} ${s*0.292} ${cx+5} ${s*0.283}`}
              stroke="#9B7A5A" strokeWidth="0.9" fill="none" strokeLinecap="round"
            />
          </>
        )}
        ...
        {/* Scar */}
        {hasScar && (
          <line x1={cx-s*0.06} y1={s*0.22} x2={cx-s*0.02} y2={s*0.29}
            stroke={tier.rankColor} strokeWidth="1" opacity="0.55" strokeLinecap="round"
          />
        )}
        ...
        {!hasHelmet ? (
          <>
            <rect x={cx-s*0.135} y={s*0.185} width={s*0.27} height={s*0.048} rx={4}
              fill={tier.bandanaColor}
            />
            {/* Metal plate */}
            <rect x={cx-s*0.075} y={s*0.189} width={s*0.15} height={s*0.035} rx={2.5}
              fill={level>=6?"#2E2E3E":"#7A7A8A"} stroke={level>=3?tier.rankColor:"#555"} strokeWidth="0.5"
            />
            {/* Plate symbol */}
            <text x={cx} y={s*0.217} textAnchor="middle" fontSize={s*0.052}
              fill={tier.rankColor} opacity="0.8"
            >{level>=15?"神":level>=10?"闇":level>=6?"忍":"木"}</text>
            {/* Bandana tail */}
            <path d={`M ${cx+s*0.135} ${s*0.205} C ${cx+s*0.17} ${s*0.22} ${cx+s*0.18} ${s*0.28} ${cx+s*0.155} ${s*0.34}`}
              stroke={tier.bandanaColor} strokeWidth="3.5" fill="none" strokeLinecap="round"
            />
          </>
        ) : (
          /* Kage conical hat */
          <>
            <ellipse cx={cx} cy={s*0.165} rx={s*0.21} ry={s*0.038} fill={tier.bandanaColor} stroke={tier.rankColor} strokeWidth="0.5"/>
            <path d={`M ${cx-s*0.09} ${s*0.165} L ${cx} ${s*0.072} L ${cx+s*0.09} ${s*0.165}`}
              fill={tier.bandanaColor} stroke={tier.rankColor} strokeWidth="0.5"
            />
            <text x={cx} y={s*0.148} textAnchor="middle" fontSize={s*0.048} fill={tier.rankColor} opacity="0.85">影</text>
            {/* Hat band */}
            <line x1={cx-s*0.21} y1={s*0.165} x2={cx+s*0.21} y2={s*0.165} stroke={tier.rankColor} strokeWidth="1" opacity="0.4"/>
          </>
        )}
        ...
        {level >= 3 && (
          <g style={{animation:"weaponGlow 2.2s ease-in-out infinite"}}>
            {level < 10 ? (
              /* Kunai — held at side */
              <g transform={`translate(${cx+26},${s*0.57}) rotate(25)`}>
                <path d={`M 0 ${-s*0.14} L -1.5 0 L 0 ${s*0.02} L 1.5 0 Z`} fill={tier.weaponColor}/>
                <rect x={-1} y={0} width={2} height={s*0.04} rx={0.5} fill="#3A2A14"/>
                <rect x={-2} y={s*0.04} width={4} height={s*0.022} rx={1} fill="#5A4020"/>
                <rect x={-0.8} y={s*0.062} width={1.6} height={s*0.05} rx={0.5} fill="#4A3520"/>
              </g>
            ) : level < 15 ? (
              /* Katana — carried diagonally */
              <g transform={`translate(${cx+18},${s*0.34}) rotate(28)`}>
                {/* Blade */}
                <path d={`M 0 ${-s*0.32} L -1.2 0 L 1.2 0 Z`}
                  fill={tier.weaponColor} style={{filter:`drop-shadow(0 0 3px ${tier.weaponColor}66)`}}
                />
                {/* Edge line */}
                <line x1={-0.3} y1={-s*0.32} x2={-0.3} y2={0} stroke="#fff" strokeWidth="0.4" opacity="0.4"/>
                {/* Guard */}
                <ellipse cx={0} cy={0} rx={5} ry={2.5} fill="#2A1A0A" stroke={tier.rankColor} strokeWidth="0.4"/>
                {/* Handle */}
                <rect x={-1.8} y={0} width={3.6} height={s*0.085} rx={1.5} fill="#3A2010"/>
                <line x1={-1.8} y1={s*0.028} x2={1.8} y2={s*0.028} stroke={tier.rankColor} strokeWidth="0.5" opacity="0.4"/>
                <line x1={-1.8} y1={s*0.056} x2={1.8} y2={s*0.056} stroke={tier.rankColor} strokeWidth="0.5" opacity="0.4"/>
              </g>
            ) : (
              /* Legendary dual blades */
              <>
                <g transform={`translate(${cx+20},${s*0.32}) rotate(25)`}>
                  <path d={`M 0 ${-s*0.36} L -1.8 0 L 1.8 0 Z`}
                    fill={tier.weaponColor} style={{filter:`drop-shadow(0 0 5px ${tier.weaponColor})`}}
                  />
                  <line x1={0} y1={-s*0.36} x2={0} y2={0} stroke="#fff" strokeWidth="0.5" opacity="0.35"/>
                  <ellipse cx={0} cy={0} rx={5.5} ry={2.5} fill="#1A0A00" stroke={tier.rankColor} strokeWidth="0.5"/>
                  <rect x={-2} y={0} width={4} height={s*0.08} rx={1.5} fill="#1A0800"/>
                </g>
                {hasDual && (
                  <g transform={`translate(${cx-20},${s*0.36}) rotate(-22)`}>
                    <path d={`M 0 ${-s*0.28} L -1.4 0 L 1.4 0 Z`}
                      fill={tier.weaponColor} opacity="0.9" style={{filter:`drop-shadow(0 0 4px ${tier.weaponColor})`}}
                    />
                    <ellipse cx={0} cy={0} rx={4.5} ry={2} fill="#1A0A00" stroke={tier.rankColor} strokeWidth="0.5"/>
                    <rect x={-1.6} y={0} width={3.2} height={s*0.07} rx={1.5} fill="#1A0800"/>
                  </g>
                )}
              </>
            )}
...
const DAYS = [
  ...
  { day:"WED", tag:"LEGS ⚠️", icon:"🦵", color:"#EF4444", focus:"Legs — Knee Protocol", kneeWarning:true, exercises:["Leg Press HIGH FEET 4x12 stop@90°","Romanian DL 4x12","Seated Leg Curl 3x15","Leg Extension PARTIAL 3x15","Hip Abductor 3x20","Glute Bridge 3x15","Calf Raise 4x15"], mobility:["Knee hug 30s x2","Clamshells 15ea","TKE band 2x15 KEY","Ham stretch 30s ea"] },
  ...
  { day:"FRI", tag:"FULL", icon:"🏃", color:"#10B981", focus:"Full Body + Mobility", exercises:["Push-Up Variations 3x15ea","RDL 3x12","Goblet Squat SHALLOW 3x12","Cable Row wide 3x12","Arnold Press 3x10","Farmer Carry 3x40m"], mobility:["Worlds greatest 5ea","Thoracic rotation 10ea","Hip flexor 30s ea","Ankle circles 10x"] },
];
...
function mondayKey() { const d = new Date(); d.setDate(d.getDate() - d.getDay() + 1); return d.toISOString().split("T")[0]; }
    ...
    <div style={{ position:"fixed", inset:0, zIndex:300, background:"rgba(0,0,0,0.9)", display:"flex", alignItems:"center", justifyContent:"center", padding:20 }} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} style={{ width:"100%", maxWidth:360, background:"linear-gradient(135deg,#1E0E04,#120508)", border:`2px solid ${tier.rankColor}`, borderRadius:20, overflow:"hidden", boxShadow:`0 0 40px ${tier.rankColor}44` }}>
        ...
        <div style={{ background:`linear-gradient(135deg,${tier.rankColor}33,${tier.rankColor}11)`, padding:"20px 20px 16px", textAlign:"center", borderBottom:`1px solid ${tier.rankColor}33` }}>
          ...
          <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:28, color:"#F0E6D3", letterSpacing:2 }}>{profile.name.toUpperCase()}</div>
          <div style={{ fontSize:11, color:"#6B4C2A", letterSpacing:2 }}>AGE {profile.age} · {profile.height}</div>
        </div>
        {/* Avatar */}
        <div style={{ padding:"20px 0 10px", display:"flex", justifyContent:"center" }}>
          <NinjaAvatar level={level} size={120}/>
        </div>
        {/* Stats grid */}
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:8, padding:"0 16px 16px" }}>
          {[
            ...
            { label:"STREAK", val:`${streak} days`, color:"#F59E0B" },
        ...
        <div style={{ padding:"0 16px 16px" }}>
          <div style={{ display:"flex", justifyContent:"space-between", fontSize:9, color:"#4A3020", marginBottom:4 }}>
          ...
          <div style={{ height:8, background:"#1A0A02", borderRadius:4, overflow:"hidden" }}>
            <div style={{ height:"100%", width:`${totalPct}%`, background:`linear-gradient(90deg,#6B728088,${arc.color})`, borderRadius:4, boxShadow:`0 0 8px ${arc.color}66` }}/>
          </div>
        </div>
        {/* Footer */}
        <div style={{ padding:"12px 16px", borderTop:`1px solid ${tier.rankColor}22`, textAlign:"center" }}>
          <div style={{ fontSize:10, color:"#4A3020", letterSpacing:2 }}>SCREENSHOT TO SHARE YOUR PROGRESS</div>
          <button onClick={onClose} style={{ marginTop:10, background:"transparent", border:`1px solid #2A1A0A`, borderRadius:8, padding:"6px 20px", color:"#4A3020", fontSize:11, cursor:"pointer", fontFamily:"'Bebas Neue',sans-serif", letterSpacing:2 }}>CLOSE</button>
        </div>
      </div>
    </div>
  );
...
export default function ShonenHQ() {
  ...
  useEffect(()=>{
    ...
    if (lastWeekReset !== mk) {
      setCheckedEx({});
      setLastWeekReset(mk);
      save("shq_lastWeekReset", mk);
      save("shq_checkedEx", {});
    }
  },[]); // eslint-disable-line react-hooks/exhaustive-deps

  // Daily reset — meals, water, supps
  useEffect(()=>{
    const dk = todayKey();
    if (lastDayReset !== dk) {
      setCheckedMeals({});
      ...
      setProToday(0);
      setLastDayReset(dk);
      save("shq_checkedMeals",{});
      save("shq_water",0);
      save("shq_supps",{});
      save("shq_calToday",0);
      save("shq_proToday",0);
      save("shq_lastDayReset",dk);
    }
  },[]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  useEffect(()=>{
    if (profile && !load("shq_weightLog",null)) {
      ...
      setWeightLog(log); save("shq_weightLog",log);
    } else if (!weightLog) {
  ...
  useEffect(()=>{
    if (xp>=xpToNext) {
      ...
      pushNotif(`LEVEL UP - ${getNinjaTier(nl).name}!`,"#F59E0B");
    }
  ...
  function logPR(key){
    const val=parseFloat(prInputs[key]);
    if(isNaN(val)||val<=0) return;
    const prev=prs[key]?.current||0;
    const isNew=val>prev;
    setPrs(p=>({...p,[key]:{current:val,baseline:p[key]?.baseline||val,best:Math.max(val,p[key]?.best||0)}}));
    setPrInputs(p=>({...p,[key]:""}));
    if(isNew){
      const bonus=Math.round((val-prev)*2);
      setXp(x=>x+25+bonus);
      setAttrs(a=>({...a,str:Math.min(99,a.str+1)}));
      pushNotif(`NEW PR! ${val}lbs! +${25+bonus} XP`,"#EF4444");
    } else {
      pushNotif(`Logged ${val}lbs`,"#6B7280");
    }
  }
  function toggleSupp(key){
    const was=supps[key];
    setSupps(s=>({...s,[key]:!s[key]}));
    if(!was){setXp(x=>x+5);pushNotif(`${SUPPS.find(s=>s.key===key)?.name} checked! +5 XP`,"#10B981");}
  }
  function addWater(){
    const next=Math.min(16,water+1);
    setWater(next);
    setXp(x=>x+3);
    pushNotif(`+1 glass! +3 XP`,"#3B82F6");
  }
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#0A0208", color:"#F0E6D3", fontFamily:"'Rajdhani',sans-serif", paddingBottom:90, position:"relative", overflow:"hidden" }}>
      ...
      {/* Share card */}
      {showShare&&<ShareCard profile={profile} weight={weight} level={level} streak={streak} attrs={attrs} arc={arc} tier={tier} totalPct={tPct} onClose={()=>setShowShare(false)}/>}
        ...
        <div style={{ maxWidth:520, margin:"0 auto" }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
            ...
            <div style={{ display:"flex", gap:8, alignItems:"center" }}>
              <button onClick={()=>setShowShare(true)} style={{ background:"#FF6B0022", border:"1px solid #FF6B0044", borderRadius:8, padding:"6px 10px", color:"#FF6B00", fontFamily:"'Bebas Neue',sans-serif", fontSize:11, letterSpacing:2, cursor:"pointer" }}>SHARE</button>
        ...
        {tab==="status"&&(
          <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
            ...
            <div style={{ background:"#0F0608", border:`1px solid ${tier.rankColor}33`, borderRadius:16, padding:"20px 16px 28px", textAlign:"center", animation:"glow 3s infinite", position:"relative", overflow:"hidden" }}>
              ...
              <div style={{ marginTop:10, fontSize:11, color:tier.rankColor, fontFamily:"'Bebas Neue',sans-serif", letterSpacing:3 }}>
            ...
            <div style={{ background:"#0F0608", border:`1px solid ${arc.color}33`, borderRadius:14, padding:"14px" }}>
              ...
              <div style={{ marginBottom:10 }}>
                ...
                <div style={{ height:8, background:"#1A0A02", borderRadius:4, overflow:"hidden" }}><div className="stat-bar" style={{ height:"100%", width:`${tPct}%`, background:`linear-gradient(90deg,#6B728088,${arc.color})`, borderRadius:4, boxShadow:`0 0 8px ${arc.color}66` }}/></div>
              </div>
              <div>
                ...
                <div style={{ height:6, background:"#1A0A02", borderRadius:3, overflow:"hidden" }}><div className="stat-bar" style={{ height:"100%", width:`${arcPct}%`, background:arc.color, borderRadius:3, boxShadow:`0 0 6px ${arc.color}` }}/></div>
              </div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
              ...
              <div style={{ background:"#0F0608", border:"1px solid #3B82F633", borderRadius:14, padding:"14px" }}>
                <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:13, color:"#3B82F6", letterSpacing:3, marginBottom:10 }}>WATER</div>
                <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:32, color:"#3B82F6", textAlign:"center", marginBottom:6 }}>{water}<span style={{ fontSize:14 }}>/8</span></div>
                <div style={{ height:4, background:"#1A0A02", borderRadius:2, overflow:"hidden", marginBottom:10 }}>
                  <div className="stat-bar" style={{ height:"100%", width:`${(water/8)*100}%`, background:"#3B82F6", borderRadius:2 }}/>
                </div>
                <button onClick={addWater} style={{ width:"100%", background:"#3B82F622", border:"1px solid #3B82F644", borderRadius:8, padding:"8px", color:"#3B82F6", fontFamily:"'Bebas Neue',sans-serif", fontSize:14, letterSpacing:2, cursor:"pointer" }}>+ GLASS</button>
                {water>0&&<button onClick={()=>setWater(w=>Math.max(0,w-1))} style={{ width:"100%", marginTop:4, background:"transparent", border:"none", color:"#4A3020", fontSize:10, cursor:"pointer" }}>undo</button>}
              </div>

              {/* Supps */}
              <div style={{ background:"#0F0608", border:"1px solid #10B98133", borderRadius:14, padding:"14px" }}>
                <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:13, color:"#10B981", letterSpacing:3, marginBottom:10 }}>SUPPS</div>
                <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
                  {SUPPS.map(s=>{
                    ...
                    return <div key={s.key} onClick={()=>toggleSupp(s.key)} style={{ display:"flex", gap:8, alignItems:"center", cursor:"pointer" }}>
                      ...
                      <div style={{ flex:1 }}>
                        <div style={{ fontSize:11, fontWeight:700, color:done?s.color:"#8B7355", textDecoration:done?"line-through":"none" }}>{s.name}</div>
                        <div style={{ fontSize:9, color:"#4A3020" }}>{s.time}</div>
                      </div>
                    </div>;
                  })}
                </div>
            ...
            <div style={{ background:"#0F0608", border:"1px solid #1A0A02", borderRadius:14, padding:"14px" }}>
              ...
              {BASE_ATTRS.map(attr=>(
                <div key={attr.key} style={{ marginBottom:10 }}>
                  ...
                  <div style={{ height:4, background:"#1A0A02", borderRadius:2, overflow:"hidden" }}><div className="stat-bar" style={{ height:"100%", width:`${Math.min(100,attrs[attr.key])}%`, background:`linear-gradient(90deg,${attr.color}55,${attr.color})`, borderRadius:2 }}/></div>
                </div>
              ))}
            </div>
            ...
            <div style={{ background:"#0F0608", border:"1px solid #EF444433", borderRadius:12, padding:"12px 14px" }}>
              <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:13, color:"#EF4444", letterSpacing:3, marginBottom:5 }}>INJURY STATUS</div>
              <div style={{ fontSize:12, color:"#8B7355", lineHeight:1.6 }}>No deep squats Wks 1-4. Goblet: 60 to 70 to 90 degrees progressive. TKEs every leg day. Ice 10 min post leg day.</div>
            </div>

            {/* Reset */}
            <button onClick={()=>{if(window.confirm("Reset all progress and start over?")){ localStorage.clear(); window.location.reload();}}} style={{ background:"transparent", border:"1px solid #2A1A0A", borderRadius:8, padding:"8px", color:"#4A3020", fontSize:11, cursor:"pointer", fontFamily:"'Bebas Neue',sans-serif", letterSpacing:2, width:"100%" }}>RESET PROFILE</button>
          </div>
        )}
        ...
        {tab==="train"&&(
          <div>
            ...
            <div style={{ background:`linear-gradient(135deg,${day.color}20,${day.color}05)`, border:`1px solid ${day.color}44`, borderRadius:14, padding:"14px 16px", marginBottom:12 }}>
              ...
              {day.kneeWarning&&<div style={{ background:"#EF444415", border:"1px solid #EF444430", borderRadius:8, padding:"7px 10px", fontSize:11, color:"#FCA5A5" }}>MENISCUS: High foot on press. Stop at 90 degrees. No jumping.</div>}
              {(()=>{ const done=day.exercises.filter((_,i)=>checkedEx[`${trainDay}-${i}`]).length; return <div style={{ marginTop:10 }}><div style={{ display:"flex", justifyContent:"space-between", fontSize:9, color:day.color, marginBottom:3 }}><span>SESSION</span><span>{done}/{day.exercises.length}</span></div><div style={{ height:4, background:"#1A0A02", borderRadius:2, overflow:"hidden" }}><div className="stat-bar" style={{ height:"100%", width:`${(done/day.exercises.length)*100}%`, background:day.color, borderRadius:2, boxShadow:`0 0 6px ${day.color}` }}/></div></div>; })()}
            </div>
            ...
            {trainView==="workout"&&(
              <div style={{ display:"flex", flexDirection:"column", gap:7 }}>
                {day.exercises.map((ex,i)=>{
                  ...
                  return <div key={i} onClick={()=>toggleEx(i)} style={{ background:done?`${day.color}14`:"#0F0608", border:`1px solid ${done?day.color+"55":"#1A0A02"}`, borderRadius:10, padding:"11px 14px", display:"flex", alignItems:"center", gap:10, cursor:"pointer" }}>
                    ...
                    <div style={{ flex:1 }}><div style={{ fontSize:13, fontWeight:700, color:done?day.color:"#F0E6D3", textDecoration:done?"line-through":"none" }}>{name}</div>{sets&&<div style={{ fontSize:10, color:done?day.color+"77":"#4A3020", letterSpacing:1 }}>{sets}</div>}</div>
            ...
            {/* PR Tracker */}
            <div style={{ marginTop:16, background:"#0F0608", border:"1px solid #EF444433", borderRadius:14, padding:"14px" }}>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                {PR_LIFTS.map(lift=>{
                  ...
                  return <div key={lift.key} style={{ background:"#0A0208", border:`1px solid ${lift.color}22`, borderRadius:12, padding:"12px" }}>
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
                      <div style={{ display:"flex", gap:8, alignItems:"center" }}>
                        <span style={{ fontSize:18 }}>{lift.icon}</span>
                        <div>
                          <div style={{ fontSize:13, fontWeight:700, color:"#F0E6D3" }}>{lift.name}</div>
                          {pr&&<div style={{ fontSize:10, color:lift.color }}>BEST: {pr.best} lbs {gained>0&&<span style={{ color:"#10B981" }}>+{gained.toFixed(1)} from start</span>}</div>}
                          {!pr&&<div style={{ fontSize:10, color:"#4A3020" }}>No PR logged yet</div>}
                        </div>
                      </div>
                      {pr&&<div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:22, color:lift.color }}>{pr.current}</div>}
                    </div>
                    <div style={{ display:"flex", gap:6 }}>
                      <input type="number" value={prInputs[lift.key]||""} onChange={e=>setPrInputs(p=>({...p,[lift.key]:e.target.value}))} placeholder="Enter lbs" style={{ flex:1, background:"#150A02", border:`1px solid ${lift.color}33`, borderRadius:6, padding:"7px 10px", color:"#F0E6D3", fontSize:14, fontWeight:700, fontFamily:"'Rajdhani',sans-serif" }}/>
                      <button onClick={()=>logPR(lift.key)} style={{ background:`${lift.color}22`, border:`1px solid ${lift.color}44`, borderRadius:6, padding:"7px 14px", color:lift.color, fontFamily:"'Bebas Neue',sans-serif", fontSize:13, letterSpacing:1, cursor:"pointer" }}>LOG</button>
                    </div>
                  </div>;
                })}
              </div>
              ...
              <>
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:7 }}>
                  {MEALS.map((m,i)=>{
                    ...
                    return <div key={i} onClick={()=>toggleMeal(i)} style={{ background:done?"#0A1A0A":"#0F0608", border:`1px solid ${done?"#10B98133":"#1A0A02"}`, borderRadius:10, padding:"11px 14px", display:"flex", alignItems:"center", justifyContent:"space-between", cursor:"pointer" }}>
                      <div style={{ display:"flex", gap:10, alignItems:"center" }}>
                        ...
                        <div>
                          <div style={{ display:"flex", gap:5, alignItems:"center" }}><span style={{ fontSize:15 }}>{m.icon}</span><span style={{ fontSize:13, fontWeight:700, color:done?"#10B981":"#F0E6D3", textDecoration:done?"line-through":"none" }}>{m.name}</span>{m.gf&&<span style={{ fontSize:10, color:"#F59E0B" }}>GF</span>}</div>
                ...
                <div style={{ background:"#0F0608", border:"1px solid #F59E0B22", borderRadius:10, padding:"10px 14px", fontSize:11, color:"#8B7355" }}>
                  GF Dinner: Same meal, plate 25-30% more for yourself. Hit 2,500 cal before dinner.
                </div>
              </>
            )}
            {fuelTab==="grocery"&&(
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
            ...
            <div style={{ background:"#0F0608", border:"1px solid #1A0A02", borderRadius:14, padding:"14px" }}>
              ...
              {ARCS.map((a,i)=>{
                ...
                return <div key={i} style={{ display:"flex", gap:10, marginBottom:9, alignItems:"center", opacity:unlocked?1:0.25 }}>
                  <div style={{ width:32, height:32, borderRadius:"50%", border:`2px solid ${a.color}`, background:complete?`${a.color}33`:"transparent", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}><span style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:14, color:a.color }}>{a.rank}</span></div>
            ...
            <div style={{ background:"#0F0608", border:"1px solid #1A0A02", borderRadius:14, padding:"14px" }}>
              ...
              {NINJA_TIERS.map((t,i)=>{
                ...
                return <div key={i} style={{ display:"flex", gap:10, marginBottom:9, alignItems:"center", opacity:unlocked?1:0.25 }}>
                  <div style={{ width:32, height:32, borderRadius:"50%", border:`2px solid ${t.rankColor}`, background:unlocked?`${t.rankColor}22`:"transparent", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0, boxShadow:unlocked?`0 0 8px ${t.rankColor}44`:"none" }}><span style={{ fontSize:10, fontWeight:900, color:t.rankColor }}>Lv{t.minLevel}</span></div>
                  <div style={{ flex:1 }}><span style={{ fontSize:14, fontWeight:800, color:t.rankColor, fontFamily:"'Bebas Neue',sans-serif", letterSpacing:2 }}>{t.name}</span><div style={{ fontSize:10, color:"#4A3020" }}>{["Kunai, Basic wrap, Green headband","Blue vest armor, Metal plate headband","Purple cloak, Dual arm wraps, Katana","Black ANBU mask, Glowing eyes, Dual blades","Kage hat, Golden blade, Battle aura","Orbiting ring, Legend weapon, Flame aura"][i]}</div></div>
